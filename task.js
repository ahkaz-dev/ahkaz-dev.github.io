...
function makeNoteInteractive(note) {
  ...
  // Only enable draggable on desktop (non-touch devices)
  ...
  if (pinBtn) {
    // Handle click and touchstart on pin button to toggle pin
  ...
  note.addEventListener('touchstart', (e) => {
    if (e.target.closest('.pin-btn')) {
      // Pin button touch handled separately, skip drag start
      return;
    }
    draggedNote = note;
    note.classList.add('dragging');
  ...
  note.addEventListener('touchend', (e) => {
    if (draggedNote) {
      ...
      // If no move and short tap, open modal
